...
// Public CORS proxies (fallback chain) - corsproxy.io is more reliable for raw content
const CORS_PROXIES = [
  'https://corsproxy.io/?',
  'https://api.allorigins.win/get?url=',  // Returns JSON wrapper, needs parsing
];
...
async function fetchWithCorsProxy(url: string): Promise<string | null> {
  for (const proxy of CORS_PROXIES) {
    try {
      ...
      // allorigins.win returns JSON wrapper, corsproxy.io returns raw content
