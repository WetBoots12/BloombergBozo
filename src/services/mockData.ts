...
// Cache for generated price history (per-session)
const priceHistoryCache = new Map<string, ChartDataPoint[]>();
...
// Lazy generation helper - generates on first access, caches for session
function getOrCreatePriceHistory(symbol: string, basePrice: number, days: number, volatility = 0.02): ChartDataPoint[] {
  const cached = priceHistoryCache.get(symbol);
  if (cached) return cached;
  const data = generatePriceHistory(basePrice, days, volatility);
  priceHistoryCache.set(symbol, data);
  return data;
}
...
// Lazy-generated chart data - generated on first access, cached per session
export const MOCK_CHART_DATA: Record<string, ChartDataPoint[]> = new Proxy({}, {
  get(_target, prop: string) {
    const chartConfigs: Record<string, { basePrice: number; days: number; volatility?: number }> = {
      AAPL: { basePrice: 189.50, days: 365 },
      MSFT: { basePrice: 415.23, days: 365 },
      GOOGL: { basePrice: 175.84, days: 365 },
      NVDA: { basePrice: 875.39, days: 365, volatility: 0.035 },
      TSLA: { basePrice: 248.50, days: 365, volatility: 0.04 },
      SPY: { basePrice: 527.82, days: 365, volatility: 0.01 },
      QQQ: { basePrice: 448.93, days: 365, volatility: 0.012 },
    };

    if (prop in chartConfigs) {
      const config = chartConfigs[prop];
      return getOrCreatePriceHistory(prop, config.basePrice, config.days, config.volatility);
    }
    return undefined;
  },
});
