...
export async function getInsiderTrades(ticker: string): Promise<InsiderTrade[]> {
  ...
  try {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      // Try to fetch the Form 4 XML to extract actual trade data
      const tradeData: {
        transactionType: InsiderTrade['transactionType'];
        shares: number;
      ...
      // Fetch Form 4 XML if it's an XML document
      if (primaryDoc.endsWith('.xml') || primaryDoc.endsWith('.txt')) {
        try {
          const xmlUrl = `https://www.sec.gov/Archives/edgar/data/${parseInt(cik)}/${accessionDashed}/${primaryDoc}`;
          const xmlResponse = await http.get(xmlUrl, { responseType: 'text' });
          const xmlText = xmlResponse.data;

          // Parse transaction code from XML
          const transCodeMatch = xmlText.match(/<transactionCode>([PSADMG])<\/transactionCode>/);
          if (transCodeMatch && ['P', 'S', 'A', 'D', 'M'].includes(transCodeMatch[1])) {
            tradeData.transactionType = transCodeMatch[1] as InsiderTrade['transactionType'];
          }

          // Parse shares traded
          const sharesMatch = xmlText.match(/<transactionShares>\s*<value>([\d,]+)<\/value>/);
          if (sharesMatch) {
            tradeData.shares = parseInt(sharesMatch[1].replace(/,/g, ''));
          }

          // Parse price per share
          const priceMatch = xmlText.match(/<transactionPricePerShare>\s*<value>([\d.]+)<\/value>/);
          if (priceMatch) {
            tradeData.pricePerShare = parseFloat(priceMatch[1]);
          }

          // Parse shares owned following transaction
          const ownedMatch = xmlText.match(/<postTransactionAmounts>\s*<value>([\d,]+)<\/value>/);
          if (ownedMatch) {
            tradeData.sharesOwned = parseInt(ownedMatch[1].replace(/,/g, ''));
          }

          // Calculate total value if we have shares and price
          if (tradeData.shares > 0 && tradeData.pricePerShare > 0) {
            tradeData.totalValue = tradeData.shares * tradeData.pricePerShare;
          }
        } catch {
          // XML parsing failed, use defaults
        }
      }

      // Extract owner name from filer data
      const filerData = data.filings?.recent?.filers?.[i];
      const ownerName = filerData?.name || recent.primaryDocDescription?.[i] || 'Insider';
      const ownerTitle = filerData?.relationship?.isOfficer ? 'Officer' :
                         filerData?.relationship?.isDirector ? 'Director' :
                         filerData?.relationship?.isTenPercentOwner ? '10% Owner' :
                         'Officer/Director';
